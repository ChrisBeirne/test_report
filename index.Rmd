...
# Import the oc trail system
trails<- st_read("shapefiles/CO_trails_system_2016.shp")
trails <- st_transform(trails, 4326)
trails2<- st_read("shapefiles/trails.shp")
#trails <- st_transform(trails, 4326)
trails3<- st_read("shapefiles/Nance_trail.shp")
...
addPolylines(data = trails2, color = "black", weight=2,opacity=1) #%>% 
...
# New map
ids <- tmp_cr_48_shp$name[tmp_cr_48_shp$taxon_canonical_name %in% c("Puma concolor", "Panthera onca", "Leopardus pardalis")]
i <- 1
m <- leaflet() %>%
  # Add a satellite image layer
  addProviderTiles(providers$CartoDB.PositronNoLabels, group="Simple") %>%
