...
The following maps relate to just the last 48 hours of data. 

### Costa Rica

```{r, echo=F, message=F, warning=F}
# Subset to 48 hours
tmp_cr_48 <- dat[dat$country=="costa_rica" & dat$timestamp > max(animals$timestamp_end, na.rm=T)-48*60*60,]
...
```{r}
#Tapir
tmp <- getMovebankData(study=1954804459, login=loginStored,  removeDuplicatedTimestamps=TRUE)
...
leaflet() %>%
  ...
  addPolylines(data=tmp_shp, color = "black",
               label = tmp_shp$NAME, group="Track",  weight=2, opacity = 0.3) %>%
