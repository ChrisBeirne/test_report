...
This document summarizes the movement locations for all active collars within Osa Conservation's `OC Vultures` project. 
...
We currently have `r length(unique(tmp_cr$name))` vultures transmitting data in Costa Rica:
...
We currently have `r length(unique(tmp_peru$name))` vultures transmitting data in Peru:
...
We currently have `r length(unique(tmp_peru$name))` vultures transmitting data in Costa Rica:
...
sub_dat_df <- left_join(sub_dat_df, animals[, c("tag_id", "name")])
...
cr_cols <- rbind(cr_cols, cr_cols[cr_cols$name=="Robin Hood",])
cr_cols$name <- as.character(cr_cols$name)
cr_cols$name[cr_cols$name=="Robin Hood"][1] <- "Robin.Hood"
