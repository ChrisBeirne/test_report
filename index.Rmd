...
# Run brownian bridge
dbbmm <- brownian.bridge.dyn(object=data2, location.error=50, dimSize=500, ext=2,
...
```{r}
tmp <- dat[dat$local_identifier=="Wilson",]
tmp$hour <- ymd_h(substr(tmp$timestamp, 1,13))
tmp2 <- tmp %>% group_by(hour) %>% summarise(n())
plot(tmp2$`n()`~ tmp2$hour)
