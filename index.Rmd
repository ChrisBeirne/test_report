  ...
  for (i in seq(along=ids))
    {
      m <- m %>%
        ...
        addMarkers(lng=last(tmp_cr$location_long[tmp_cr$animalName==ids[i]]),
                   ...
                   icon = leafIcons) #, fillOpacity = 1, opacity = 1, radius=5, color = tmp_cr_shp$colours[i], group = ids[i], stroke=F
    }
  ...
  for (i in seq(along=ids))
    {
      m <- m %>%
        ...
        addMarkers(lng=last(tmp_cr_48$location_long[tmp_cr_48$animalName==ids[i]]),
                   ...
                   icon = leafIcons) #, fillOpacity = 1, opacity = 1, radius=5, color = tmp_cr_48_shp$colours[i], group = ids[i], stroke=F
    }
...
m <- leaflet() %>%
  ...
  addProviderTiles(providers$OpenStreetMap.Mapnik, group="OS") #%>%     
  m <- m %>%
        addPolylines(data = night_shp, color = night_shp$colours, weight=3,opacity=0.3, popup=night_shp$name) %>% 
        addCircleMarkers(lng=first_and_last$location_long,
                         lat=first_and_last$location_lat, 
                         popup=paste("name"=first_and_last$name, "<br>Time = ", substr(first_and_last$timestamp,12,16)),
