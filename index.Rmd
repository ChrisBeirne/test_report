...
last_obs <- dat %>% group_by(animalName) %>% dplyr::summarize(hours_since_fix=min(diff_time))
...
tmp_sum <- dat %>% group_by(animalName) %>% dplyr::summarise(total_obs=n(), mean_batt=mean(eobs_battery_voltage, na.rm=T), loc_accuracy=mean(eobs_horizontal_accuracy_estimate, na.rm=T))
#colnames(dat)
...
names <- sub('\\_.*', '', tmp_cr_shp$animalName)
i <- 1
m <- leaflet() %>%
  ...
  for (i in seq(along=ids))
    {
      m <- m %>%
        ...
        # Add the last point
        addMarkers(lng=last(tmp_cr$location_long[tmp_cr$animalName==ids[i]]),
                         lat=last(tmp_cr$location_lat[tmp_cr$animalName==ids[i]]), 
                         popup=paste(last(tmp_cr$local_identifier[tmp_cr$animalName==ids[i]]),
                                     "<br>Time:",last(tmp_cr$timestamp[tmp_cr$animalName==ids[i]]),
                                     "<br>Long:",last(tmp_cr$location_long[tmp_cr$animalName==ids[i]]),
                                     "<br>Lat:",last(tmp_cr$location_lat[tmp_cr$animalName==ids[i]])),
                   icon = leafIcons, group=ids[i]) #, fillOpacity = 1, opacity = 1, radius=5, color = tmp_cr_shp$colours[i], group = ids[i], stroke=F
    }
...
tmp_cr2 <- tmp_cr %>% group_by(animalName, hour, .drop=F) %>% dplyr::summarize(obs=n())

# Tag activity - account for survey effort
tmp_act_cr <- tmp_cr %>% group_by(animalName, .drop=F) %>% dplyr::summarize(start=min(timestamp), end=max(timestamp))
...
tmp_peru2 <- tmp_peru %>% group_by(animalName, hour, .drop=F) %>% dplyr::summarize(obs=n())
      ...
      m <- m %>%
        ...
        addMarkers(lng=last(tmp_cr_48$location_long[tmp_cr_48$animalName==ids[i]]),
                         ...
                         popup=paste(last(tmp_cr_48$local_identifier[tmp_cr_48$animalName==ids[i]]),
                                     ...
                                     "<br>Lat:",last(tmp_cr_48$location_lat[tmp_cr_48$animalName==ids[i]])),
                   icon = leafIcons, group=ids[i]) #, fillOpacity = 1, opacity = 1, radius=5, color = tmp_cr_48_shp$colours[i], group = ids[i], stroke=F
    }
...
ids <- unique(sub_dat_df$animalName)  

  for(i in 1:length(ids))      
        # The last point
  m <-   m %>%   addMarkers(lng=last(sub_dat_df$location_long[sub_dat_df$animalName==ids[i]]),
                         lat=last(sub_dat_df$location_lat[sub_dat_df$animalName==ids[i]]), 
                         popup=paste(last(sub_dat_df$local_identifier[sub_dat_df$animalName==ids[i]]),
                                     "<br>Time:",last(sub_dat_df$timestamp[sub_dat_df$animalName==ids[i]]),
                                     "<br>Long:",last(sub_dat_df$location_long[sub_dat_df$animalName==ids[i]]),
                                     "<br>Lat:",last(sub_dat_df$location_lat[sub_dat_df$animalName==ids[i]])),
                   icon = leafIcons)
...
```{r, echo=F, warning=F, message=F}
 ...
 tmp <- tmp[order(tmp$timestamp.first),]
...
lfc <- do.call(st_sfc,
              lapply(split(first_and_last, first_and_last$name),
                     function(d){st_linestring(as.matrix(d[,c("location_long", "location_lat")]))}))
night_shp <- st_sf(data.frame(animalName=levels(factor(first_and_last[,"name"])), geom=lfc))
...
cr_cols <- tmp_cr[, c("animalName", "colours")]
cr_cols <- cr_cols[duplicated(cr_cols)==F,]
cr_cols$animalName<- sub('\\_.*', '', cr_cols$animalName)

cr_cols <- rbind(cr_cols, cr_cols[cr_cols$animalName=="Robin Hood",])
cr_cols$animalName <- as.character(cr_cols$animalName)
cr_cols$animalName[cr_cols$animalName=="Robin Hood"][1] <- "Robin.Hood"
 
night_shp <- left_join(night_shp, cr_cols)

#night_dat_df <- left_join(night_dat_df, cr_cols, by=c("name"= "animalName"))
first_and_last <- left_join(first_and_last, cr_cols, by=c("name"= "animalName"))

tmp <- first_and_last %>% group_by(name) %>%  dplyr::summarise(nights=n_distinct(night_id))
night_shp <- left_join(night_shp, tmp, by=c("animalName"="name"))
...
night_shp_df <- night_shp_df[,c("animalName", "roost_spacing_km"),]
...
night_shp_df[,c("animalName", "roost_spacing_km")] %>%
  kbl(row.names = FALSE) %>%
  kable_styling()  %>% 
...
m <- leaflet() %>%
  ...
  addProviderTiles(providers$OpenStreetMap.Mapnik, group="OS") #%>%     
  m <- m %>%
        addPolylines(data = night_shp, color = night_shp$colours, weight=3,opacity=0.3, popup=night_shp$name, group=night_shp$animalName) %>% 
      ...
      addLayersControl(
        baseGroups = c("OS", "Simple", "Satellite"),
        overlayGroups = night_shp$animalName,
        options = layersControlOptions(collapsed = FALSE)
      )  #%>%
     #addLegend(position= "topright", colors=night_shp$colours, 
      #          labels=night_shp$animalName ,opacity = 0.7, title = "Animals") 
