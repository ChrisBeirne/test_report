...
harness <- mov_dat[mov_dat$name %in% c("Santa", "Cleopatra"),]
...
harness_shp$colours <- c("orange", "purple")
...
nine_pms <- night[substr(night$timestamp,12,13)==21,]
...
for(i in 1:length(unique(tmp3$name)))
{
  ...
  if(tmp4$distance_to_fence[1]<8){ tmp4$GPS_check[1] <- 1}
...
Show the full table just to check the counter is working!
