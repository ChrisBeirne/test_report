...
# Clean up the name
animals$animalName <- paste0(sub('\\_.*', '', animals$animalName), "_", sub('\\ .*', '', animals$individual_taxon_canonical_name))
...
mov_dat <- moveStack(tmp1, tmp3, tmp4, tmp5)
...
dat$animalName <- paste0(dat$animalName, "_", sub('\\ .*', '', dat$individual_taxon_canonical_name))
...
dat$icon <- sub(".*? ", "", dat$individual_taxon_canonical_name)
...
sp_dat <- dat[,c("name",    "country", "individual_taxon_canonical_name" )]
...
tmp <- last_sum[, c("name","individual_taxon_canonical_name","country", "timestamp", "diff_time")]
...
fig %>% add_pie(data = dplyr::count(check_in_summary, individual_taxon_canonical_name), labels = ~individual_taxon_canonical_name, values = ~n,
...
data <- t(as.data.frame.matrix(table(check_in_summary$individual_taxon_canonical_name, check_in_summary$country)))
...
# Last 48 hours
# Vulture
tmp_cr_48 <- dat[dat$country=="costa_rica" & dat$timestamp > max(animals$timestamp_end, na.rm=T)-48*60*60 & dat$individual_taxon_canonical_name %in% c("Cathartes aura" ,"Cathartes melambrotus", "Sarcoramphus papa", "Coragyps atratus"),]
#Mammals
tmp_48 <- dat[dat$country=="costa_rica" & dat$timestamp > max(animals$timestamp_end, na.rm=T)-336*60*60 & !(dat$individual_taxon_canonical_name %in% c("Cathartes aura" ,"Cathartes melambrotus", "Sarcoramphus papa", "Coragyps atratus")),]
...
#table(dat$individual_taxon_canonical_name)
# Remake shapefiles
lfc <- do.call(st_sfc,
...
sub_dat_df <- left_join(sub_dat_df, animals[, c("tag_id", "name", "individual_taxon_canonical_name")])

# Subset to just vultures
sub_dat_df <- sub_dat_df[sub_dat_df$individual_taxon_canonical_name %in% c("Cathartes aura", "Sarcoramphus papa", "Coragyps atratus", "Cathartes melambrotus"),]


sub_dat_df <- sub_dat_df[order(sub_dat_df$name),]

# Add in icon column
sub_dat_df$icon <- sub(".*? ", "", sub_dat_df$individual_taxon_canonical_name)
...
sub_dat <- sub_dat[sub_dat$name %in% animals$name[animals$individual_taxon_canonical_name %in% c("Cathartes aura", "Sarcoramphus papa", "Cathartes melambrotus" )],]
